...
class V2Ichannels:
    ...
    def get_shadowing(self, delta_distance, shadowing):
        ...
        z1 = 1 - np.exp(-2 * (delta_distance / self.Decorrelation_distance))
        z2 = np.sqrt(z1)
        z3 = np.multiply(np.exp(-1 * (delta_distance / self.Decorrelation_distance)), shadowing) \
               + np.sqrt(1 - np.exp(-2 * (delta_distance / self.Decorrelation_distance))) * np.random.normal(0, 8, nVeh)
        z4 = np.exp(-1 * (delta_distance / self.Decorrelation_distance))
...
class Environ:
    ...
    def renew_channels_fastfading(self):
        ...
        x1 = np.random.normal(0, 1, V2I_channels_with_fastfading.shape) + 1j * np.random.normal(0, 1, V2I_channels_with_fastfading.shape)
        x2 = np.abs(x1)
        x3 = x2 / math.sqrt(2)
        x4 = -20 * np.log10(x3)
        ...
        xxxx = V2I_channels_with_fastfading + x4
        fff = 2
